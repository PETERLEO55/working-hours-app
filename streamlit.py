...
if st.button('Calculate'):
    ...
    for i,j in zip(start, end):
        t1 = datetime.combine(now.date(),datetime.strptime(i, '%I:%M:%S %p').time(),ist)
        t2 = datetime.combine(now.date(),datetime.strptime(j, '%I:%M:%S %p').time(),ist)
        total_sec += (t2-t1).total_seconds()
    if len(start) > len(end):
        last_in = datetime.combine(now.date(),datetime.strptime(start[-1], '%I:%M:%S %p').time(),ist)
